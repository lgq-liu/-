...
@WebServlet(name = "BlackCtListServlet", urlPatterns = "/BlackCtListServlet")
public class BlackCtListServlet extends HttpServlet {
    @Override
    public void doGet(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        List<Contact> contactList = new ArrayList<>(); //用于存放黑名单联系人的列表
        int pageSize=5;
        int currentPage = (req.getParameter("currentPage") != null) ? Integer.parseInt(req.getParameter("currentPage")) : 1;
        int totalContactPage = DBUtil.getTotalContactPage("10001", pageSize, 1); //获取总页数
        int start = (currentPage - 1) * pageSize; //计算查询起始位置
        ...
        try{
            ...
            if(req.getParameter("sort")!=null){ //判断有无排序请求
                ...
                if(!(sex.equals("all"))){   //判断有无性别筛选
                    ...
                    sql="SELECT ct_id,ct_name, ct_mf, ct_phone FROM t_contact  WHERE ct_delete=1 AND ct_mf='"+sex+ "' ORDER BY ct_name "+sort+ " LIMIT ?,?";
                }
                else{
                    sql="SELECT ct_id,ct_name, ct_mf, ct_phone FROM t_contact  WHERE ct_delete=1 ORDER BY ct_name "+sort+ " LIMIT ?,?";
                }
            }
            else{
                ...
                if(!(sex.equals("all"))) {   //判断有无性别筛选
                    ...
                    sql = "SELECT ct_id,ct_name, ct_mf, ct_phone FROM t_contact where ct_delete=1 AND ct_mf='"+sex+"' LIMIT ?,?";
                }
                else{
                    sql = "SELECT ct_id,ct_name, ct_mf, ct_phone FROM t_contact where ct_delete=1 LIMIT ?,?";
                }
            }
            if((keyword!=null)&&(keyword!="")){  //最后判断是否有搜索操作
                sql="SELECT ct_id,ct_name, ct_mf, ct_phone FROM t_contact where ct_delete=1 AND ct_name='"+keyword+"'";
            }
        ...
        if(fail){
            ...
            req.setAttribute("path","BlackCtListServlet");
            RequestDispatcher rd = req.getRequestDispatcher("views/Error.jsp");
            rd.forward(req, resp);
        }
        else{
            req.setAttribute("contactList",contactList);
            RequestDispatcher rd = req.getRequestDispatcher("views/BlackCtList.jsp"); //请求下一个jsp页面
